...
void add_require(v8::Isolate * isolate, const v8::Local<v8::ObjectTemplate> & object_template, std::vector<std::string> & paths)
{
	...
	(void)add_function(isolate, object_template, "require", [paths](const v8::FunctionCallbackInfo<v8::Value> & info, std::string filename)->v8::Local<v8::Value>{
		...
		for(auto path : paths) {
			try {
				...
				isolate_require_results.insert(std::pair<std::string, v8::Global<v8::Value>&>(complete_filename, *new_global));
