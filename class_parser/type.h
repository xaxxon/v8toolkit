...
private:
    ...
    static std::string convert_simple_typename_to_jsdoc(std::string simple_type_name, std::string const & = "");
...
public:
    ...
    /**
     * returns non-const version of this type (may be the same if not const to begin with)
    ...
    // returns the root incldues for this type (including any template type parameters it may have)
